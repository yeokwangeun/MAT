...
MIDAS_DIR = os.path.dirname(os.path.realpath(__file__))
...
def get_model(optimize=False, height=None, square=False):
    model_type = "dpt_beit_large_512"
    model_path = os.path.join(MIDAS_DIR, "weights/dpt_beit_large_512.pt")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model, transform, net_w, net_h = load_model(device, model_path, model_type, optimize, height, square)
    params = {
        "transform": transform,
...
def get_depth(img, params):
    if img.ndim == 2:
        img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB) / 255.0
    image = params["transform"]({"image": img})["image"]
